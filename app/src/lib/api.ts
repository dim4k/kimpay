...
export async function createKimpay(
    name: string,
    icon: string,
    creatorName: string,
    ...
    try {
        ...
        await pb.collection("kimpays").update(
            kimpayRecord.id,
            {
...
export async function addParticipant(kimpayId: string, name: string) {
    return await pb.collection("participants").create<Participant>(
        {
            name,
            kimpay: kimpayId,
        },
...
export async function updateKimpay(
    id: string,
...
export async function createReimbursement(
    ...
    toId: string,
    amount: number,
    description: string = "Reimbursement"
) {
    const data = {
        kimpay: kimpayId,
    ...
    // Touch the kimpay to trigger realtime update
    await pb
        .collection("kimpays")
        .update(kimpayId, { updated: new Date() }, { requestKey: null });
...
export async function updateParticipant(
    id: string,
    data: Partial<{ name: string; avatar: File | null }>
) {
    return await pb
