<script lang="ts">
  ...
  async function handleExport(format: 'csv' | 'md') {
      try {
          ...
          if (format === 'csv') {
              // CSV Header
              content += "Date,Description,Amount,Payer,For\n";
              
              expenses.forEach((e: any) => {
                  const date = new Date(e.date).toLocaleDateString();
                  const desc = `"${(e.description || '').replace(/"/g, '""')}"`;
                  const amount = e.amount.toFixed(2);
                  const payer = e.expand?.payer?.name || "Unknown";
                  
                  // Get Involved Names
                  const involvedIds = e.involved || [];
                  const involvedNames = involvedIds.map((id: string) => participantMap.get(id) || "Unknown").join('; ');
              ...
              content += `**Participants:** ${participantsList.map((p: any) => p.name).join(', ')}\n\n`;
              
              content += `## Expenses\n\n`;
              content += `| Icon | Date | Description | Amount | Payer | For |\n`;
              content += `|:---:|---|---|---|---|---|\n`;
